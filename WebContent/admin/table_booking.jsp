...
small">Administrator</span>
					...
					<div class="card shadow mb-4">
						...
						<div class="card-body">
							<div class="table-responsive">
								<table id="tb_booking" class="table table-bordered" width="100%"
									cellspacing="0">
								<thead>
									<tr>
										<th>ID</th>
	...
	<script>
		var tbBooking = document.getElementById('tb_booking');
		...
		databaseRef.once('value', function(snapshot) {
			snapshot.forEach(function(childSnapshot) {
				...
				var row = tbBooking.insertRow(rowIndex);
				var cellId = row.insertCell(0);
				var cellCarname = row.insertCell(1);
				var cellCarprice = row.insertCell(2);
				var cellFullname = row.insertCell(3);
				var cellEmail = row.insertCell(4);
				var cellPhonenumber = row.insertCell(5);
				var cellPickupaddress = row.insertCell(6);
				var cellDate1 = row.insertCell(7);
				var cellDropaddress = row.insertCell(8);
				var cellDate2 = row.insertCell(9);
				var cellCardname = row.insertCell(10);
				var cellCardnumber = row.insertCell(11);
				var cellTotal = row.insertCell(12);
				
				
				cellId.appendChild(document.createTextNode(childKey));
				cellCarname.appendChild(document.createTextNode(childData.carname));
				cellCarprice.appendChild(document.createTextNode(childData.carprice));
				cellFullname.appendChild(document.createTextNode(childData.fullname));
				cellEmail.appendChild(document.createTextNode(childData.email));
				cellPhonenumber.appendChild(document.createTextNode(childData.phonenumber));
				cellPickupaddress.appendChild(document.createTextNode(childData.pickupaddress));
				cellDate1.appendChild(document.createTextNode(childData.date1));
				cellDropaddress.appendChild(document.createTextNode(childData.dropaddress));
				cellDate2.appendChild(document.createTextNode(childData.date2));
				cellCardname.appendChild(document.createTextNode(childData.cardname));
				cellCardnumber.appendChild(document.createTextNode(childData.cardnumber));
				cellTotal.appendChild(document.createTextNode(childData.total));
				rowIndex = rowIndex + 1;
			});
			var table = document.getElementById("tb_booking");
